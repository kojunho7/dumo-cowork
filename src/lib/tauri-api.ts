...
export async function getSettings(): Promise<Settings> {
  if (!isTauri()) {
    ...
    const stored = localStorage.getItem("dumo-cowork-settings");
    if (stored) {
...
export async function listConversations(): Promise<Conversation[]> {
  if (!isTauri()) {
    const stored = localStorage.getItem("dumo-cowork-conversations");
...
export async function createConversation(title: string): Promise<Conversation> {
  if (!isTauri()) {
    ...
    localStorage.setItem("dumo-cowork-conversations", JSON.stringify(conversations));
    return conv;
  }
...
export async function deleteConversation(id: string): Promise<void> {
  if (!isTauri()) {
    ...
    localStorage.setItem("dumo-cowork-conversations", JSON.stringify(filtered));
    localStorage.removeItem(`dumo-cowork-messages-${id}`);
    return;
  }
...
export async function getMessages(conversationId: string): Promise<Message[]> {
  if (!isTauri()) {
    const stored = localStorage.getItem(`dumo-cowork-messages-${conversationId}`);
    return stored ? JSON.parse(stored) : [];
  }
...
function saveMessagesLocal(conversationId: string, messages: Message[]) {
  localStorage.setItem(
    `dumo-cowork-messages-${conversationId}`,
...
export async function listTasks(): Promise<Task[]> {
  if (!isTauri()) {
    const stored = localStorage.getItem("dumo-cowork-tasks");
    return stored ? JSON.parse(stored) : [];
  }
...
export async function createTask(
  ...
  if (!isTauri()) {
    ...
    const tasks = await listTasks();
    tasks.unshift(task);
    localStorage.setItem("dumo-cowork-tasks", JSON.stringify(tasks));
    return task;
  }
...
export async function deleteTask(id: string): Promise<void> {
  if (!isTauri()) {
    ...
    localStorage.setItem("dumo-cowork-tasks", JSON.stringify(filtered));
    return;
  }
...
export async function getTaskMessages(taskId: string): Promise<TaskMessage[]> {
  if (!isTauri()) {
    // Web fallback
    const stored = localStorage.getItem(`dumo-cowork-task-messages-${taskId}`);
    return stored ? JSON.parse(stored) : [];
  }
